...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const SIZE_CHARTS = {
        ...
        regular: {
            ...
            rows: { XS: [104, 38, 44, 19, 18], S: [105, 41, 46, 20, 20], M: [106, 44, 48, 20, 22], L: [108, 47, 50, 21.5, 22] }
        },
        bermudas: {
    ...
    // Valor del radio del checkout → literal que se guarda en `ordenes.tipo_envio`.
    //
    // ⚠️ Estas cadenas TIENEN que coincidir con `src/lib/envios.ts`. El front no
    // puede importar de `src/`, así que la coincidencia la garantiza un test
    // (`tests/tipos-envio.test.ts`), no el compilador.
    //
    // `retiro` se agregó el 2026-08-22: retiro coordinado en mano, sin cargo y
    // sin OCA. Es el único tipo que vale $0.
    const TIPOS_ENVIO = {
        domicilio: 'puerta_puerta',
        sucursal: 'sucursal',
        retiro: 'retiro_local',
    };
    ...
    async function _populateConfirmationFromAPI(ordenId) {
        ...
        try {
            ...
            const envioEl = document.getElementById('confirmacion-envio-value');
            if (envioEl) {
                const tipoEnvio = orden.tipo_envio === TIPOS_ENVIO.retiro ? 'Retiro coordinado'
                    : orden.tipo_envio === TIPOS_ENVIO.sucursal ? 'OCA — Sucursal'
                    : 'OCA — Domicilio';
                ...
                if (ordenId) {
                    try {
                        ...
                        const patchRes = await fetch(`/api/ordenes/${ordenId}`, {
                            ...
                            body: JSON.stringify({
                                tipo_envio: TIPOS_ENVIO[selectedEnvio.value] || TIPOS_ENVIO.sucursal,
    ...
    async function cargarOpcionesEnvioOCA(cpDestino) {
        ...
        try {
            ...
            // Tercera opción, propia: retiro coordinado sin cargo.
            agregarOpcionRetiro(containerEl);
            ...
            existingOptions.forEach(el => el.style.opacity = '1');
            // Aunque OCA no responda, el retiro coordinado sigue siendo válido:
            // no depende de ellos. Sin esto, si se cae la cotización el cliente
            // se queda sin ninguna opción utilizable.
            agregarOpcionRetiro(metodoEnvioSection.querySelector('.checkout-section-header')?.parentElement);
        }
    }

    /**
     * Agrega la opción de retiro coordinado sin cargo al final de la lista.
     *
     * Reusa las clases del resto de las opciones — CERO CSS nuevo.
     *
     * `data-precio` va en centavos como las de OCA (el handler del Step 2 hace
     * `parseFloat(dataset.precio) / 100`), y `0` es el único precio de envío
     * legítimo del sistema: el backend lo valida contra este mismo tipo.
     *
     * Hoy es visible para todos (decisión de Naza, 2026-08-22). Para limitarla
     * más adelante alcanza con condicionar esta llamada — la opción no existe en
     * el HTML estático, así que si no se agrega acá, no está en ningún lado.
     */
    function agregarOpcionRetiro(containerEl) {
        if (!containerEl) return;
        if (containerEl.querySelector('[data-tipo="retiro"]')) return; // idempotente

        containerEl.insertAdjacentHTML('beforeend', `
            <div class="envio-opcion" data-tipo="retiro" data-precio="0">
                <input type="radio" name="metodo-envio" id="envio-retiro" value="retiro">
                <label for="envio-retiro" class="envio-opcion-label">
                    <div class="envio-opcion-header">
                        <span class="envio-opcion-nombre">Coordinar retiro</span>
                        <span class="envio-opcion-precio">Sin cargo</span>
                    </div>
                    <div class="envio-opcion-detalle">
                        <span class="envio-opcion-plazo">Coordinamos lugar y horario por mail</span>
                    </div>
                </label>
            </div>
        `);
