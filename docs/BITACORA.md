...
- **Problema encontrado:** varios frentes en paralelo antes del lanzamiento en Instagram (19:00). Los precios de agosto había que reflejarlos en tres lugares (Supabase, el catálogo del front y el vault). Las tablas de talle de las PDP eran **placeholders**: progresiones aritméticas perfectas (+2 de largo, +3 de pecho) que no salían de medir ninguna prenda. Las descripciones de las 4 INTERVENCIONES no aprovechaban el ancho del contenedor. El bloque CUIDADO cambiaba de tipografía en mobile.
- **Solución adoptada:** migración 22 con los precios nuevos, corrida y verificada al centavo contra el front con `verificar:catalogo`. Facturación potencial: **$44.425.000 → $52.695.000 (+18,6%)** sobre el mismo inventario; el denim pasa de 22-36% a 36-47% de margen. Naza midió el depósito y la modista confirmó el escalado (+1 cm de largo, +1 por lado de ancho): se verificó con dos talles medidos en oversize y boxy, se cumple, y el resto se extrapoló. Las letras de las tablas ahora **siguen al mockup** (B es el largo, C la manga) y "Hombro" pasó a "Ancho". Cada pieza 1/1 tiene tabla propia, resuelta por slug exacto **antes** que los prefijos — sin eso `bermuda-camo-woodland` caía en la tabla de las bermudas de producción. Las descripciones se arreglaron sacando **22 `<br>` hardcodeados**: las 4 intervenciones eran los únicos productos que los tenían.
- **Archivo modificado:** `public/js/start.js`, `src/app/globals.css`, `backend/sql/22_precios_agosto_2026.sql`. Commits `8117ed2`, `8ab7422`, `30d1fbf`.
- **Pendiente:** medidas reales de musculosa y bermudas de producción (siguen placeholder, pero sus PDP están bloqueadas por `RESTRICTED_CATEGORIES`). Descripciones de las dos bermudas: 16 y 23 caracteres, muy cortas para llenar una línea.

### ⚠️ El webhook de NAVE sigue sin llegar — dos compras reales lo confirman
- **Problema encontrado:** el 21-ago el E2E cobró la orden 63 y no llegó ni el mail ni el envío. **Cero requests** a `/api/webhooks/nave`, verificado con un POST de control propio que sí aparecía en los logs. La causa: la URL dada de alta en NAVE era el **apex**, que **307-redirecciona** a `www`, y los emisores de webhooks no siguen redirects en POST. Peor: `guidocapuzzi.com` (sin diéresis) sirve una **página parkeada de Hostinger** que responde 200 a un GET, así que verificar desde el browser se ve bien mientras los POST mueren. NAVE actualizó la URL el 22-ago **y sigue sin notificar**: las órdenes 63 y 66 tienen `nave_status = 'SUCCESS_PROCESSED'` y `nave_payment_id = null`, valores que **sólo escribe la red de seguridad**. El webhook escribiría `APPROVED`.
- **Solución adoptada:** se reescribió la red de seguridad del GET de `/api/ordenes/[id]`, que tenía una **dependencia circular**: exigía `nave_payment_id`, que lo setea el webhook, o sea que no podía cubrir el caso "el webhook nunca llegó" — para el que se había escrito. Corrió 7 veces sobre la orden 63 y las 7 fueron no-ops. Ahora hay dos caminos: si está el `payment_id` verifica el pago, y si no, verifica la **intención** con `nave_payment_request_id` contra `GET /api/payment_requests/{id}`. Los dos fallan cerrado. La orden 63 se reconcilió sola al desplegarlo.
- **Archivo modificado:** `src/app/api/ordenes/[id]/route.ts`, `src/lib/nave/client.ts`, `tests/red-de-seguridad-pago.test.ts`. Commit `4489eb4`.
- **Pendiente:** **el cron de conciliación.** La red actual es *pull*: la dispara el browser del cliente al volver a la confirmación. Si alguien paga y cierra la pestaña, la orden queda en `pago_pendiente` para siempre — sin stock descontado (la prenda se revende, grave en las 1/1), sin mail y sin aviso. Reclamarle a NAVE con las órdenes 63 y 66 como evidencia.

### Aviso interno de venta y tercera opción de envío
- **Problema encontrado:** Naza y Fede no tenían forma de enterarse de una compra sin entrar a Supabase. Y un conocido que quisiera comprar por la web estaba obligado a pagar envío.
- **Solución adoptada:** `sendInternalOrderNotification()` a ncgc@ y fmgc@ con cada compra pagada, colgado del **mismo claim atómico de `email_sent`** que la confirmación al cliente, así sale una sola vez sin agregar una columna. Cuando es retiro, el asunto lleva "RETIRO A COORDINAR" y el cuerpo abre con un banner rojo. Para el envío: tercera opción **sin cargo y sin OCA**, visible para todos por decisión de Naza. El cambio que importaba era un **guard en el webhook**: sin él una orden de retiro genera etiqueta real y despacha un correo. Los dos ternarios de `tipo_envio` tenían dos ramas, así que un tercer valor caía en "domicilio" y el mail le decía al cliente que su compra viajaba a la casa.
- **Archivo modificado:** `src/lib/email.ts`, `src/lib/envios.ts` (nuevo), `src/app/api/webhooks/nave/route.ts`, `public/js/start.js`. Commits `8117ed2`, `1b7a0a6`.
- **Pendiente:** NAVE **no informa cuotas ni medio de pago** en ningún endpoint documentado, así que el aviso lo dice explícitamente en vez de afirmar "contado". Se amplió a 2500 chars el log del payload de `payment_requests` para ver si vienen ahí en la próxima venta.

### Los emails seguían en Univers cuatro meses después del cambio de marca
- **Problema encontrado:** `src/lib/email.ts` cargaba `UniversCnBold.ttf` y `UniversRegular.ttf` desde Supabase Storage. Pero el daño real era el **logo**: `mail_smtp.png` estaba dibujado en Univers (subido en febrero) y, al ser una imagen, **no tenía fallback que lo salvara** — era el único lugar donde la tipografía vieja seguía visible de verdad.
- **Solución adoptada:** se **sacó** el `@font-face` en vez de reemplazarlo: Gmail, Outlook y Yahoo lo strippean, y servir Helvetica Neue desde el bucket público sería exactamente el web-embedding que falta licenciar con Monotype. Logo reemplazado por `mail-logo-registrado.png`, rasterizado del SVG de marca (1000×116). **No se puede usar el SVG**: Gmail lo strippea y Outlook no lo soporta. Los dos templates de Supabase Auth se extrajeron a `docs/emails/supabase-auth/` con `{{ .ConfirmationURL }}` de verdad — la versión del preview tenía `href="#"` y URLs de muestra, o sea que pegada dejaba **los botones muertos**.
- **Archivo modificado:** `src/lib/email.ts`, `email-preview.html`, `docs/emails/supabase-auth/`. Commit `17f85dd`.
- **Pendiente:** pegar los dos templates en el panel de Supabase (manual). Licencia de Helvetica Neue con Monotype.

## 2026-08-20

### Azure DevOps conectado — CI en verde en las dos plataformas
- **Problema encontrado:** faltaba conectar el pipeline de Azure al repo. Antes de empezar apareció un riesgo que no estaba en el plan: Microsoft dejó de otorgar automáticamente los **jobs paralelos gratuitos** a organizaciones nuevas, y sin el grant el pipeline se crea, arranca y queda **encolado para siempre** (`No hosted parallelism has been purchased or granted`). Se destraba con un formulario de 2-3 días hábiles o vinculando una suscripción de Azure con billing. Se chequeó primero, antes de armar nada: la org `naza89` **sí tiene** el free tier (1 job paralelo, 1800 min/mes). Con ~5-6 min por corrida eso da más de 300 corridas mensuales, así que no hace falta optimizar los tres stages en uno.
- **Solución adoptada:** Naza creó el pipeline desde el panel (New pipeline → GitHub → `naza89/gu.idocapuzzi.com` → Existing YAML → `/azure-pipelines.yml`). El agente no pudo hacerlo: no hay Azure CLI instalado, no hay MCP de Azure DevOps, y por API haría falta un PAT. Primer run: **verde en los tres stages**, pero con **6 warnings** (dos por job): `NodeTool@0` quedó deprecada.
- **Migración a `UseNode@1`:** la trampa está en el input, que se **renombró de `versionSpec` a `version`**. Cambiar sólo el nombre de la task no explota: la task ignora el input desconocido y cae a **su default, Node 10.x** — ahí sí se rompe todo, porque el *type stripping* de TypeScript de los tests necesita Node ≥22 y Next 16 tampoco arranca en 10. Sería una falla confusa en un stage que estaba verde, por un cambio que parece cosmético. Quedó escrito en el encabezado del YAML. Se verificó la sintaxis contra la doc de Microsoft antes de tocar nada, en vez de asumirla. GitHub Actions no se tocó: `setup-node@v4` no está deprecada.
- **Archivo modificado:** `azure-pipelines.yml`. Commit `0253cca`. Segundo run: **verde y sin warnings**.
- **Pendiente:** ninguno. Queda para el día que el deploy pase por el pipeline: un Environment con approval gate para la coreografía de "código primero, migraciones después", que hoy vive sólo en la bitácora y en la memoria de Naza.


### El primer run del CI encontró que un secreto faltante tumbaba los dos webhooks
- **Problema encontrado:** el run #1 del pipeline, sobre `045ef0a`, falló en el stage Build: *"Failed to collect page data for /api/webhooks/oca"*. Lint y Test pasaron. La causa no era del CI: `src/lib/email.ts` construía el cliente **a nivel de módulo** (`const resend = new Resend(process.env.RESEND_API_KEY)`), y `new Resend(undefined)` **tira** (`"Missing API key"` — verificado ejecutándolo). Ese archivo lo importan **los dos webhooks del camino del dinero**, `/api/webhooks/nave` y `/api/webhooks/oca`, así que un throw en el import no rompe el mail: **rompe la ruta entera**. Traducido: si `RESEND_API_KEY` faltara o se rotara mal en Vercel, NAVE no podría avisar que una orden se pagó — no fallaría el mail, fallaría el procesamiento del pago. Nunca se había visto porque la variable siempre estuvo: Next carga `.env.local` en el build local, y Vercel la tiene seteada. **Hizo falta un build sin secretos para que apareciera.**
- **Solución adoptada:** `getResend()` construye y cachea el cliente en el primer uso, devuelve `null` con log si la clave no está, y cada envío chequea antes de usarlo. La ausencia ahora degrada sólo el envío del mail. Verificado con `next build` en las condiciones exactas del CI (`RESEND_API_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, las tres de NAVE y las tres de OCA vacías, sólo las dos `NEXT_PUBLIC` con placeholders): exit 0. Se sumaron 3 tests y **se verificó que fallan de verdad**: reintroduciendo el bug a propósito dan 2 rojos, sacándolo vuelven los 55 verdes. El tercero generaliza el patrón a `supabase.ts`, `nave/client.ts` y `oca/client.ts` — cualquier `new Cliente(process.env.X)` en scope de módulo es la misma trampa.
- **Archivo modificado:** `src/lib/email.ts`, `tests/invariante-precio-servidor.test.ts`. Commit `0828eef`, pusheado. Run #2 del CI: Lint, Test y Build los tres en verde. Deploy `dpl_G3JoViXv…` READY.
- **Pendiente:** ninguno.

> **La lección, que corrige lo que se había argumentado unas horas antes en esta misma sesión:** se había dicho que el stage Build no aportaba nada porque Vercel ya buildea. **Es falso.** Vercel buildea con todas las env vars puestas, así que estructuralmente **no puede** encontrar esta clase de bug. El CI, justamente por ser más pobre en secretos, es más estricto. La decisión de no darle credenciales se tomó por seguridad (la filtración de la clave de OCA del 18-ago) y terminó pagando por un motivo distinto el mismo día. Vale también como aviso sobre la verificación previa: la simulación de checkout limpio había corrido lint, typecheck, test y `verificar:catalogo` sin `.env.local`, pero el `next build` se había corrido en el repo normal, donde Next carga `.env.local` solo. Ese fue el hueco exacto por donde entró.
